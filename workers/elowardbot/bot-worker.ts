...
router.get('/debug/:channel', async (req: Request, env: Env) => {
  try {
    ...
    if (!channel) {
      return json(400, { error: 'Channel parameter required' });
    }

    const records = await env.DB.prepare(`
      SELECT twitch_id, channel_name, bot_enabled, timeout_seconds, 
             reason_template, enforcement_mode, min_rank_tier, min_rank_division,
             created_at, updated_at
      FROM twitch_bot_users 
      WHERE channel_name = ? OR LOWER(channel_name) = LOWER(?) OR channel_name LIKE ?
    `).bind(channel.toLowerCase(), channel, `%${channel}%`).all();

    return json(200, {
      channel_searched: channel,
      records_found: records.results?.length || 0,
